...
public class InputManagerScript : MonoBehaviour {
    ...
    protected GameObject selectionIconReference;
    protected GameObject currentSelectionIcon;

	public virtual void Start () {
		moveManager = GetComponent<MoveTokensScript>();
		gameManager = GetComponent<GameManagerScript>();
        selectionIconReference = Resources.Load("SelectorIcon") as GameObject;
	}
		...
		if(Input.GetMouseButtonDown(0)){
			...
			if(collider != null){
				...
				if(selected == null){
                    ...
                    currentSelectionIcon = Instantiate(selectionIconReference, selected.transform.position, Quaternion.identity) as GameObject;		
				} else {
